...
def test_translate_contract_emits_five_artifacts_per_object(sales):
    assert sorted(a.relative_path for a in sales) == [
        "expectations/sales__customer_expectations.yaml",
        "schemas/load/sales__customer_schema.yaml",
        "schemas/transform/sales__customer_schema.yaml",
        "schemas/write/sales__customer_schema.yaml",
        "tags/sales__customer_tags.yaml",
    ]
...
def test_translate_contract_emits_one_artifact_set_per_object_when_multi_object(
    ...
    assert "schemas/write/multi__orders_schema.yaml" in paths
    assert "schemas/write/multi__products_schema.yaml" in paths
...
def test_load_schema_names_columns_by_physical_name(sales):
    schema = _artifact(sales, "schemas/load/sales__customer_schema.yaml")
...
def test_transform_schema_renames_only_when_physical_name_differs(sales):
    schema = _artifact(sales, "schemas/transform/sales__customer_schema.yaml")
...
def test_transform_schema_omits_column_mapping_when_no_renames(multi_contract_path):
    ...
    schema = _artifact(artifacts, "schemas/transform/multi__orders_schema.yaml")
...
def test_write_schema_keeps_operational_metadata_and_scd2_columns(sales):
    schema = _artifact(sales, "schemas/write/sales__customer_schema.yaml")
...
def test_tags_file_maps_object_tags_with_key_value_convention(sales):
    tags = _artifact(sales, "tags/sales__customer_tags.yaml")
...
def test_tags_file_has_empty_tags_when_object_declares_none(multi_contract_path):
    ...
    tags = _artifact(artifacts, "tags/multi__orders_tags.yaml")
...
def test_expectations_emit_not_null_when_property_required(sales):
    exp = _artifact(sales, "expectations/sales__customer_expectations.yaml")
